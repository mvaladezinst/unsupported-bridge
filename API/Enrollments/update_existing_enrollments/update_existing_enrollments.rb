...
csv_file = nil # file path to CSV containing learner enrollment ID, score, and completion date
...
CSV.foreach(csv_file, {:headers => true}) do |row|
  Typhoeus.patch("#{base_url}/enrollments/#{row['id']}",
                params: { 'enrollments[0][score]' => row['score'], 'enrollments[0][completed_at]' => row['completed_at'] },
                headers: {'Content-Type'=>'application/json', 'accept'=>'application/json', 'Authorization'=>"Basic #{access_token}"}
  )
  puts "#{row['id']}, #{row['score']}, #{row['completed_at']}"
end
