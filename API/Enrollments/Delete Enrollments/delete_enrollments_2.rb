...
# Michael A. Valadez
# 12.02.16
# Publish Courses - Publishes Courses In Bulk
#=============
require 'csv'
require 'typhoeus'
require 'HTTParty'
...
CSV.foreach(csv_file, {:headers => true}) do |row|
  Typhoeus.post("#{base_url}/course_templates/#{row['id']}/publish?as_user_id=#{admin_id}",
                headers: {'Content-Type'=>'application/json', 'accept'=>'application/json', 'Authorization'=>"Basic #{access_token}"}
  )
  puts "Course #{row['id']} is now Published."
end
