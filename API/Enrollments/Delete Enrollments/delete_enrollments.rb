...
# Michael A. Valadez
# 11.13.16
# Delete Enrollments - Assumes Enrollment Already Exists
#=============
require 'csv'
require 'typhoeus'
require 'HTTParty'
...
# Path to the CSV file containing the learner enrollment ID and state.
csv_file = 'nil'
...
CSV.foreach(csv_file, {:headers => true}) do |row|
  Typhoeus.delete("#{base_url}/enrollments/#{row['id']}",
                 headers: {'Content-Type'=>'application/json', 'accept'=>'application/json', 'Authorization'=>"Basic #{access_token}"}
  )
  puts "Enrollment #{row['id']}, is now deleted."
end

puts 'Successfully deleted enrollments.'
sleep(1)
system('cls')
