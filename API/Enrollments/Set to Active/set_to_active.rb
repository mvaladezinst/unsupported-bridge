...
# Michael A. Valadez
# 11.13.16
# Set Enrollments Status - Assumes Enrollment Already Exists
#=============
...
CSV.foreach(csv_file, {:headers => true}) do |row|
  Typhoeus.patch("#{base_url}/enrollments/#{row['id']}",
                 params: { 'enrollments[0][state]' => row['state'] },
                 headers: {'Content-Type'=>'application/json', 'accept'=>'application/json', 'Authorization'=>"Basic #{access_token}"}
  )
  puts "#{row['id']}, is now #{row['state']}"
end

puts 'Successfully updated statuses.'
sleep(1)
system('cls')
