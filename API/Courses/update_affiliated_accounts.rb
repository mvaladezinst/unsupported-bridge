...
# Michael A. Valadez
# 11.13.16
# Update Course Affiliated Accounts
#=============
require 'csv'
require 'typhoeus'
require 'HTTParty'
...
CSV.foreach(csv_file, {:headers => true}) do |row|
  response = Typhoeus.put("https://#{bridge_domain}.bridgeapp.com/api/author/affiliated_sub_accounts/share",
                           headers: {'Content-Type'=>'application/json', 'accept'=>'application/json','Authorization'=>"Basic #{access_token}"},
                           params: { 'item_id' => row['item_id'],'item_type' => 'CourseTemplate','domain_id' => row['domain_id']},

  )
  puts "Sharing course ID #{row['item_id']} to sub-account ID #{row['domain_id']}"
  puts response.code
end
