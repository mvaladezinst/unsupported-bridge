...
# Michael A. Valadez
# 12.02.16
# Course Description - Updates Course Descriptions In Bulk
#=============
require 'csv'
require 'typhoeus'
require 'HTTParty'
...
CSV.foreach(csv_file, {headers: true}) do |row|
  Typhoeus.patch("#{base_url}/course_templates/#{row['id']}",
                        headers: {'Content-Type'=>'application/json', 'accept'=>'application/json', 'Authorization'=>"Basic #{access_token}"},
                        params: { course_template: { description: row['description']} }
 )

  puts "Course #{row['id']} - description updated."
end

puts 'Successfully Updated Descriptions.'
sleep(1)
system('cls')
