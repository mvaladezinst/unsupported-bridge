...
# Michael A. Valadez
# 12.02.16
# Update Learning Library - Places Courses In Learning Library
#=============
require 'csv'
require 'typhoeus'
require 'HTTParty'
...
# Admin ID for the account - Must be account admin ID.
admin_id = 'nil'
...
CSV.foreach(csv_file, {:headers => true}) do |row|
  Typhoeus.post("#{base_url}/library_items/course_templates/#{row['id']}?as_user_id=#{admin_id}",
                 headers: {'Content-Type'=>'application/json', 'accept'=>'application/json', 'Authorization'=>"Basic #{access_token}"}
  )
  puts "Course #{row['id']}, now in Learning Library."
end

puts 'Successfully updated Learning Library.'
sleep(1)
system('cls')
