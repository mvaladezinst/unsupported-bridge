...
CSV.foreach(csv_file, {:headers => true}) do |row|
 response = Typhoeus.post("#{base_url}/taggings",
                 params: { 'tags[0][context_id]' => row['context_id'],'tags[0][name]' => row['name'],'tags[0][context_type]' => 'course_template'},
                 headers: {'Content-Type'=>'application/json', 'accept'=>'application/json', 'Authorization'=>"Basic #{access_token}"}
  )
  puts "Creating Tag #{row['name']}, for course #{row['context_id']}"
  puts response.code
end
